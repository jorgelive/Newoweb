...
|---|---|---|
| `TarifaDailyPriceFlattener::flatten()` | un precio que no es número (`'abc'`) entraba como `(float) 'abc'` = **0.00**: el día salía gratis | el rango **no cuenta**, como si le faltara el precio, y el día lo rellena la tarifa base (o se omite, que es el contrato de siempre, §12.0.1). Igual con un fallback sin precio legible. ⚠️ **Sólo en el push a Beds24** (`Beds24RatesPushQueueCreator`, que le pasa el precio crudo): `PmsTarifaCalculadora::preciosPorNoche()` y los providers del calendario convierten a `float` ANTES de llamar, así que ahí un `'abc'` ya llega como `0.0`. Es teórico —`PmsTarifaRango::$precio` es `DECIMAL NOT NULL`—, pero si algún día entra texto, el calendario y los cargos seguirían en 0.00 |
