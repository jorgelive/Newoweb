...
2. **Poblar `pms_conversacion_enlace`** desde el `contextType`/`contextId`/`contextData`
   actual, **y decidir quién refresca el enlace cuando la reserva cambia**: hoy
   `PmsReservaRecalculoService` reescribe el `contextData` de la conversación; cuando existan
   enlaces tiene que reescribir el enlace (milestones, vínculo, status_tag) en el mismo
   movimiento, o el motor programará con fechas viejas.
3. ~~Enseñarle a `MessageRuleEngine` a programar por ASUNTO~~ — **hecho** (§20.5). Las columnas
   viejas siguen siendo la verdad para las conversaciones sin enlaces.
4. Fusionar los hilos duplicados por contacto, con su historial. Los mensajes viajan con su
   asunto estampado; los sin estampar deben adoptarse ANTES de mover (o moverse junto con las
   columnas de contexto), porque la deducción usa el par de la conversación de origen.
5. Retirar `contextType`/`contextId`/`contextData` de la conversación. Antes de esto, TODOS los
   mensajes vivos deben estar estampados (la adopción es perezosa: un barrido con
   `app:message:sync-rules --all` la completa).
6. Decisión de producto pendiente: si el **aviso de cancelación por asunto** vuelve, y con qué
   guarda contra las canceladas duplicadas.

Los pasos 1 y 2 se pueden desplegar sin que nada cambie. El 4 es el que hay que medir.
