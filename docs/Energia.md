# Energía — consumo eléctrico medido por enchufe

Cómo se mide, se imputa y se cobra la electricidad que gasta un huésped, con enchufes
inteligentes Tuya y una bitácora horaria que se le puede enseñar.

**Alcance:** `src/Energia/` (dominio) y el cliente de Tuya en `src/Exchange/Service/Client/`
...
⚠️ **El nombre del módulo se queda corto y conviene decidirlo antes de la migración.** El parque no
es sólo de contómetros: hay aparatos que sólo conmutan, y el accionamiento llegará. `Domotica`
describe mejor lo que es, y hoy renombrar cuesta un `sed` — con la migración aplicada cuesta una
migración de renombrado en producción. Ver §9.
...
```
  EnergiaDispositivo      el enchufe físico, atado a una PmsUnidad
          │
          │  ManyToOne
          ▼
  EnergiaSuscripcion      dispositivo ↔ evento de calendario
          │               consumoTotal · consumoCliente   ← ESTADO ACTUAL (proyección)
          │
          │  ManyToOne
          ▼
  EnergiaLectura          una fila por hora
...
nunca al revés.** Se puede recalcular entera recorriendo `EnergiaLectura`; existe sólo para no
...
**Dependencia dura a `App\Pms`.** `EnergiaDispositivo` apunta a `PmsUnidad` y
`EnergiaSuscripcion` a `PmsEventoCalendario` con FK de verdad, saliéndose de la pauta de
...
`EnergiaDispositivo` no es «un contómetro»: es un aparato con dos capacidades que se declaran por
...
⚠️ `EnergiaDispositivoRepository::mudos()` y `lecturaVencida()` filtran por `mideConsumo`. Un
...
`EnergiaLectura::$incremento` guarda los kW·h del tramo aunque se puedan deducir restando filas
...
`consumoCliente` a cero, marcada con `EnergiaMotivoLectura::Reinicio` y su nota. El salto en la
...
`EnergiaMotivoLectura::reiniciaConsumoCliente()` es quien decide qué motivos ponen a cero;
...
`EnergiaSuscripcion::diasDeCredito()` cuenta **días empezados, no completos**: quien entra a las
...
parámetros de `services_energia.yaml` son sólo la semilla. El precio del kW·h lleva años quieto,
...
`EnergiaDispositivo::lecturaVencida()` y `EnergiaDispositivoRepository::mudos()`: el cron es
...
| Servicio de muestreo | `src/Energia/Service/` | Toma la última lectura y le suma la diferencia de contadores |
| Cron horario | `src/Energia/Command/` | Recorre `EnergiaSuscripcionRepository::vigentes()` |
| ~~Alerta de mudos~~ | `EnergiaVigilarCommand` + `VigilanteDeDispositivos` | **Hecho.** Ver abajo |
| Accionamiento on/off | `src/Energia/Service/` | `conmutable` ya está modelado; falta el `POST` a Tuya y quién puede pulsarlo |
| Renombrar el módulo a `Domotica` | todo `src/Energia/` | **Decidir antes de la migración**: después cuesta renombrar tablas en producción |
| Panel de dispositivos | `util/` | Alta, asignación a unidad, reinicio a solicitud |
| Vista del huésped | `pax/` | El extracto de `EnergiaLectura::comoLinea()` |
...
|---|---|---|
| Cambiar la tarifa del kW·h | `.env` / `.env.local` | `ENERGIA_TARIFA_SOLES_KWH` — sólo afecta a suscripciones nuevas |
| Cambiar la cortesía diaria | `.env` / `.env.local` | `ENERGIA_CREDITO_DIARIO_SOLES` — ídem |
| Cambiar cómo se cuentan los días de crédito | `src/Energia/Entity/EnergiaSuscripcion.php` | `diasDeCredito()` |
| Cambiar qué se cobra de verdad | `src/Energia/Entity/EnergiaSuscripcion.php` | `importeACobrarSoles()` |
| Añadir un motivo de lectura | `src/Energia/Enum/EnergiaMotivoLectura.php` | + `reiniciaConsumoCliente()` y `exigeNota()` |
| Cambiar cada cuántos fallos se avisa | `config/services/services_energia.yaml` | `energia.fallos_para_alerta` (3) — módulo, avisa en el 3, 6, 9… |
| Cambiar el barrido por reloj | `config/services/services_energia.yaml` | `energia.horas_para_alerta` (3) |
| Cambiar quién recibe la alerta | `src/Energia/Service/VigilanteDeDispositivos.php` | `destinatarios()` — hoy `Roles::OPERACIONES_SHOW` |
| Cambiar el texto del extracto del huésped | `src/Energia/Entity/EnergiaLectura.php` | `comoLinea()` — vive en la entidad para que app y asistente digan lo mismo |
| Enchufar un aparato nuevo | Panel (pendiente) | Clave natural: `EnergiaDispositivo::$tuyaDeviceId` |
...
| **Por reloj** | `app:energia:vigilar` | `lecturaTomadaEn` | **Que se muera el muestreo entero** |
...
**Se avisa cada N fallos, no cada fallo.** `EnergiaDispositivo::debeAlertar()` usa un módulo: salta
...
perdido se recupera pidiendo el rango otra vez, y `uniq_energia_lectura_hora` hace que reescribir
