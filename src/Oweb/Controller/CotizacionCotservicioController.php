...
class CotizacionCotservicioController extends CRUDController
{
    ...
    public function clonarAction(Request $request): Response
    {
        ...
        if(!empty($request->query->get('cotizacion_id'))){
            ...
            $cotizacion = $this->entityManager
                ->getRepository('App\Oweb\Entity\CotizacionCotizacion')
        ...
        foreach($object->getCotcomponentes() as $cotcomponente):
            if(isset($newFechaInicio) && isset($interval)) {
                ...
                $cotcomponente->setEstadocotcomponente($this->entityManager->getReference(CotizacionEstadocotcomponente::class, CotizacionEstadocotcomponente::DB_VALOR_PENDIENTE));
            }
            $cotcomponente->setEstadocotcomponente($this->entityManager->getReference(CotizacionEstadocotcomponente::class, 1));
        endforeach;

        $this->admin->create($newObject);

        return new RedirectResponse($this->generateUrl('admin_app_oweb_cotizacioncotizacion_edit', ['id' => $cotizacion->getId()]));
    }
    ...
    public function icalAction(Request $request): Response
    {
        ...
        $calendar = $this->icalGenerator->setTimezone('America/Lima')->setProdid('-//OpenPeru//CotizacionCotservicio Calendar //ES')->createCalendar();
