        ...
        $builder
            ->add('is_active', CheckboxType::class, [
                ...
                'help' => AyudaPlegable::html(
                    'Decide si el canal de WhatsApp <b>se ofrece siquiera</b> para esta plantilla — a mano y al agente.',
            ...
            ->add('is_official_meta', CheckboxType::class, [
                ...
                'required' => false,
                'help' => AyudaPlegable::html(
                    'Decide si puede salir <b>fuera</b> de la ventana de 24 h.',
                    'Desmárcalo si no está aprobada en Meta (un «Quick Reply» interno, o una plantilla que sólo '
                    . 'existe como texto libre). Sin esto, fuera de la ventana el envío se rechaza con un aviso '
                    . '— que es lo correcto: ahí hay que mandar una aprobada.'
                ),
                'help_html' => true,
                'row_attr' => ['class' => 'col-md-12 mb-3'],
            ])
            ->add('meta_template_name', TextType::class, [
                ...
                'help' => AyudaPlegable::html(
                    'El nombre exacto aprobado en Meta. Sin él la plantilla es <b>invisible en las dos '
                    . 'direcciones</b>: ni se sube ni se reconoce al sincronizar.',
                    '⚠️ <b>Ponle sufijo desde el primer día</b> (<code>pago_v1</code>). Meta no deja editar una '
                    . 'plantilla aprobada: reescribirla es crear otra y borrar la vieja, y <b>borrar bloquea ese '
                    . 'nombre 30 días</b>. Con sufijo, la siguiente es <code>_v2</code> y no te topas con el bloqueo.'
                ),
                'help_html' => true,
                'row_attr' => ['class' => 'col-md-12 mb-3'],
            ])
            ...
            ->add('body', CollectionType::class, [
                ...
                'help' => AyudaPlegable::html(
                    '<b>Sólo se usa FUERA de la ventana de 24 h.</b> Dentro sale el de «Enlace WhatsApp», más largo y libre.',
                    '<b>Límites de Meta:</b> 1024 caracteres; el texto no puede empezar ni acabar con una variable, '
                    . 'ni llevar dos seguidas; y <b>ningún parámetro puede tener saltos de línea</b>. Por eso '
                    . '<code>{{bloque_pago}}</code> no cabe aquí —son varias líneas— y sí cabe '
                    . '<code>{{importe_a_pagar}}</code>, que es una.<br><br>'
                    . 'El estado (Aprobada/Pendiente) lo sincroniza Meta. Las variables se convierten al formato '
                    . 'posicional al enviarlas.'
                ),
                'help_html' => true,
                'allow_add' => true,
                'allow_delete' => true,
