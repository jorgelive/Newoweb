 ...
 * Contra {@see CruceDeFrontera}, que es el «set de instrucciones» que faltaba: el vuelo con el que
 * ESA persona entra al país y el vuelo con el que sale, sacados de su propio subgrupo aéreo. Y
 * contra el pasaporte de su ficha.
 *
 * | campo | qué dice el documento | contra qué |
 * |---|---|---|
 * | `pasaporte` | el número que declaró al rellenarlo | su ficha del manifiesto |
...
final readonly class CotejoDeEticket
{
     ...
     * @param string|null     $pasaporte  el de su ficha; `null` = no tiene pasaporte cargado
     */
    public static function de(DatosDeEticket $leido, CruceDeFrontera $cruce, ?string $pasaporte = null): self
    {
        ...
        if ($pasaporte !== null && $leido->pasaporte !== null
            && !self::mismoTexto($leido->pasaporte, $pasaporte)) {
            $diferencias[] = new Discrepancia('pasaporte', $leido->pasaporte, $pasaporte);
        }
        ...
        if ($pasaporte === null) {
            $notas[] = 'no tiene pasaporte en el manifiesto: no se pudo comprobar a nombre de quién está';
        }
